    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("loginKorisnicko") == null) {  //register request;  kreira korisnika i poziva registruj() nad njim 
            try {
                ...
                request.setAttribute("msg", "Connection error, please try again");
                request.getRequestDispatcher("registracija.jsp").forward(request, response);
                return;
            }
            request.setAttribute("msgTip", "uspeh");
            request.setAttribute("msg", "Successful registration, please login!");
            ...
            try {                
                if (loginKorisnik.login()) {
                    ...
                    request.setAttribute("msg", "Wrong password or username!");
                    request.getRequestDispatcher("login.jsp").forward(request, response);
                }                  
            } catch (SQLException sqle) {
                request.setAttribute("msgTip", "greska");
                request.setAttribute("msg", "Internal error, please try again!");
            }
        }
    }
