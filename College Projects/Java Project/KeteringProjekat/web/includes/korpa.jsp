...
<c:if test="${Narudzbina == null}">
    <div class="prazna-korpa">
        <div class="poruka-korpa">
            <p style="font-family: 'Montserrat Black', sans-serif;">The cart is empty!</p>
            <p><a href="Pocetna" class="btn btn-dark" style="font-family: 'Montserrat Black', sans-serif;">Back to home</a></p>
        </div>
    </div>
</c:if>
    ...
    <form class="korpa-prikaz" action="Narucivanje" method="post">
            ...
            <c:forEach var="stavka" items="${Narudzbina.getStavkeNarudzbine().keySet()}">
                <div class='korpa-stavka'>
                    ...
                    <div class="stavka-info">
                        ...
                        <p class='stavka-total'>Price: ${stavka.getCenaPoPorciji()} $</p>
                    </div>
                    <div class="stavka-buttons">
                        <a href="" id="a${stavka.getProizvodID()}" class="btn btn-warning btn-stavka">Change</a>
                        <a href="Korpa?Zahtev=Izbrisi&Proizvod=${stavka.getProizvodID()}" class="btn btn-danger btn-stavka">Delete</a>
                    </div>
                </div>
            </c:forEach>
        ...
        <div class="korpa-footer">
            <input class="btn btn-light poeni-korpa" type="number" min="0" id="poeni" onchange='popust(this)' max="<c:if test="${sessionScope.Poeni>2}">2</c:if><c:if test="${sessionScope.Poeni<=2}">${sessionScope.Poeni}</c:if>" name="poeni" placeholder="Poeni">
                ...
                <div class="total">
                        <p>Total: <span id="subtotal">${Narudzbina.getTotalBezPopusta()}</span> RSD</p> 
                <p>Total with discount: <span id="total">${Narudzbina.getTotalBezPopusta()}</span> RSD</p>
            </div>
        </div>
