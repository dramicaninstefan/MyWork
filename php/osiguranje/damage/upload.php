...
$reg_oznaka_stetnik = isset($_POST['reg_oznaka_stetnik']) ? $conn->real_escape_string($_POST['reg_oznaka_stetnik']) : null;  
$broj_polise_stetnik = isset($_POST['broj_polise_stetnik']) ? $conn->real_escape_string($_POST['broj_polise_stetnik']) : null;  
...
$vrsta_stete = isset($_POST['vrsta_stete']) ? $conn->real_escape_string($_POST['vrsta_stete']) : null;  
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($vrsta_stete !== null) {
        $updates[] = "vrsta_stete = '$vrsta_stete'";
    }
    ...
    if ($reg_oznaka_stetnik !== null) {  // Dodato da se ažurira registarska oznaka
        $updates[] = "reg_oznaka_stetnik = '$reg_oznaka_stetnik'";
    }
    
    if ($broj_polise_stetnik !== null) {  // Dodato da se ažurira registarska oznaka
        $updates[] = "broj_polise_stetnik = '$broj_polise_stetnik'";
    }
    ...
    if ($emin_procena_disabled !== null) {  // Dodato da se ažurira registarska oznaka
        $updates[] = "emin_procena_disabled = '$emin_procena_disabled'";
    }

    if ($sluzbena_beleska_disabled !== null) {  // Dodato da se ažurira registarska oznaka
        $updates[] = "sluzbena_beleska_disabled = '$sluzbena_beleska_disabled'";
    }
