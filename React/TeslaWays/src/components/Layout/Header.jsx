...
const Header = () => {
  ...
  return (
    <Fragment>
      <header>
        <nav>
          <ul>
            ...
            <li className={classes.navLinks}>
              <NavLink className={({ isActive }) => (isActive ? classes.active : undefined)} to="/lokacije" end>
        ...
        <div className={classes['hamburger-icon']}>
          <label htmlFor="check" className={classes.bar} ref={navBtnRef}>
            <input id="check" type="checkbox" onClick={handleClick} checked={isClicked} onChange={(current) => !current} />
            <span className={classes.top}></span>
            <span className={classes.middle}></span>
            <span className={classes.bottom}></span>
          </label>
          <ul ref={navRef} className={isClicked ? `${classes['mobile-menu']} ${classes.open}` : classes['mobile-menu']}>
            <li className={classes.navLinks}>
              <NavLink className={({ isActive }) => (isActive ? classes.active : undefined)} onClick={isClicked} to="/" end>
                {t('Pocetna')}
              </NavLink>
            </li>
            <li className={classes.navLinks}>
              <NavLink className={({ isActive }) => (isActive ? classes.active : undefined)} to="/lokacije" onClick={isClicked} end>
                <i className="fas fa-location-dot"></i>
                {t('Lokacije')}
              </NavLink>
            </li>
            <li className={classes.navLinks}>
              <NavLink className={({ isActive }) => (isActive ? classes.active : undefined)} onClick={isClicked} to="/vesti" end>
                <i className="fa-solid fa-newspaper"></i>
                {t('Novosti i zanimljivosti')}
              </NavLink>
            </li>
            ...
            <li className={classes.navLinks}>
              <NavLink className={({ isActive }) => (isActive ? classes.active : undefined)} onClick={isClicked} to="/o_nama" end>
                {t('O nama')}
              </NavLink>
            </li>
            <li className={classes.navLinks}>
      ...
      <div ref={menuRef} className={isOpen ? `${classes['side-bar-container']} ${classes.open}` : classes['side-bar-container']}>
        <div className={classes['side-bar']}>
          <div className={classes.menu}>
            <h2 className={classes.title}>{t('Zone')}</h2>

            {zones?.map((niz) => (
              <ZoneList key={niz.naziv} naziv={niz.naziv} zona={niz.id_zone} api={destinations} refDivEl={addToDivRef} refH3El={addToH3Ref} />
            ))}
          </div>
        </div>
      </div>
